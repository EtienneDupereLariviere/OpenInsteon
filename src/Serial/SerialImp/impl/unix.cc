...
Serial::SerialImpl::SerialImpl (const string &port, unsigned long baudrate,
                                ...
                                void (*signalDataReceived)(int sig, siginfo_t *sinfo, void *value))
  : port_ (port), fd_ (-1), is_open_ (false), xonxoff_ (false), rtscts_ (false),
    baudrate_ (baudrate), parity_ (parity),
    bytesize_ (bytesize), stopbits_ (stopbits), flowcontrol_ (flowcontrol),
    dataReceived_ (signalDataReceived)
{
...
Serial::SerialImpl::reconfigurePort ()
{
  ...
  saio.sa_sigaction = dataReceived_;
...
#error "OS Support seems wrong."
  ...
  if (stopbits_ == stopbits_t::stopbits_one_point_five) {
    byte_time_ns_ += ((1.5 - stopbits_t::stopbits_one_point_five) * bit_time_ns);
  }
}
